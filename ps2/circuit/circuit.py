...
class PriorityQueue:
    ...
    def __init__(self):
        ...
        self.queue = [None] # first elem not used
        self.min_index = None
    ...
    def pop(self):
        ...
        while popped_index * 2 < len(self.queue):
            left_child = popped_index * 2
            right_child = popped_index * 2 + 1

            if (right_child >= len(self.queue)) or (self.queue[right_child] > self.queue[left_child]):
                self.queue[popped_index] = self.queue[left_child]
                popped_index = left_child
            else:
                self.queue[popped_index] = self.queue[right_child]
                popped_index = right_child
        
        last_node_index = len(self.queue) - 1
        self.queue[popped_index] = self.queue[last_node_index]

        self.heapify(popped_index)
